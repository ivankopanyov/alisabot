...
def test_append_service(client, db, admin):
...
def test_append_no_exists_service(client, db, admin):
    response = login_user(client, email=ADMIN_EMAIL)
    assert "access_token" in response.json
    access_token = response.json["access_token"]
    response = create_position(client, access_token, position_name=DEFAULT_NAME)
    assert response.status_code == HTTPStatus.CREATED
    position_id = response.json["id"]
    response = append_service_to_position(client, access_token, position_id=position_id, service_id=0)
    assert response.status_code == HTTPStatus.NOT_FOUND


def test_append_no_exists_position(client, db, admin):
    response = login_user(client, email=ADMIN_EMAIL)
    assert "access_token" in response.json
    access_token = response.json["access_token"]
    response = create_service(client, access_token, service_name=DEFAULT_NAME)
    assert response.status_code == HTTPStatus.CREATED
    service_id = response.json["id"]
    response = append_service_to_position(client, access_token, position_id=0, service_id=service_id)
    assert response.status_code == HTTPStatus.NOT_FOUND


def test_append_service_no_admin_token(client, db, admin, user):
    response = login_user(client, email=ADMIN_EMAIL)
    assert "access_token" in response.json
    access_token = response.json["access_token"]
    response = create_position(client, access_token, position_name=DEFAULT_NAME)
